...
export { listLanguageModels } from './providers';
...
export async function getModelInstance(model: string): Promise<LanguageModel> {
  const modelInstance = await getLanguageModel(model);
  return modelInstance.createClient();
}
