            ...
            >
              <div
                className={cn('w-12 shrink-0', {
                  'text-muted-foreground': ['in_progress', 'loading_packages'].includes(consoleOutput.status),
                  ...
                  <div className="text-muted-foreground">
                    {consoleOutput.status === 'in_progress'
                      ...
                      : consoleOutput.status === 'loading_packages'
                        ? consoleOutput.contents.map((content) => (content.type === 'text' ? content.value : null))
                        : null}
                  </div>
                </div>
              ) : (
                  ...
                  {consoleOutput.contents.map((content, index) =>
                    content.type === 'image' ? (
                      <picture key={`${consoleOutput.id}-${index}`}>
                        <img src={content.value} alt="output" className="max-w-screen-toast-mobile w-full rounded-md" />
                      </picture>
                    ) : (
                      <div key={`${consoleOutput.id}-${index}`} className="w-full whitespace-pre-line break-words">
                        {content.value}
                      </div>
                    )
                  )}
